...
Class Munk_Sites {
    ...
    function admin_scripts( $id ){
        if( $id == 'appearance_page_munk-sites' ){
            wp_localize_script('jquery', 'Munk_Sites',  $this->get_localize_script() );
            ...
            wp_enqueue_style('munk-sites', MUNK_SITES_URL.'/assets/css/munk-sites.css' );

            wp_enqueue_script('owl.carousel', MUNK_SITES_URL.'/assets/js/owl.carousel.min.js',  array( 'jquery' ), false, true );
            wp_enqueue_script('munk-sites', MUNK_SITES_URL.'/assets/js/backend.js',  array( 'jquery', 'underscore' ), false, true );
        }
    }
    ...
    static function get_api_url(){
        return apply_filters( 'munk_sites/api_url', 'https://metricthemes.github.io/munk-instant-sites/sites.json' );		
    }
    ...
    function page(){
        echo '<div class="wrap">';
        echo '<h1 class="wp-heading-inline">'.__( 'Munk Sites', 'munk-sites' ).'</h1><hr class="wp-header-end">';
